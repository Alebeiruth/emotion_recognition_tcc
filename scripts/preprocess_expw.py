...
BASE_PATH = r'C:\Users\luhal\TCC\data\processed\expW_organized'
# O novo output é um dataset limpo, mas ainda desbalanceado
OUTPUT_PATH = r'C:\Users\luhal\TCC\data\processed\expw_aligned_gray'  
TARGET_SIZE = (224, 224)

# Inicializar o detetor de rosto MTCNN
detector = MTCNN()

# --- FUNÇÃO AUXILIAR DE ALINHAMENTO (Exatamente a mesma de antes) ---
...
print(f"Iniciando pré-processamento do ExpW...")
...
for split in ['train', 'test']:
    ...
    if not os.path.exists(input_split_path):
        print(f"AVISO: Pasta de entrada não encontrada, pulando: {input_split_path}")
        continue

    # A principal mudança está aqui: iteramos diretamente pelas pastas com os nomes das emoções
    for emotion_name in tqdm(os.listdir(input_split_path), desc=f"Processando {split}"):
        input_emotion_path = os.path.join(input_split_path, emotion_name)
        output_emotion_path = os.path.join(output_split_path, emotion_name)
        
        # Verificar se é um diretório antes de processar
...
print("\nPré-processamento (limpeza e padronização) do ExpW concluído com sucesso!")